...
type FlexServicePoolGroup struct {
	poolMap    map[string]map[string]*ServicePool // poolMap[groupId][ipAndPort]=new(ServicePool)
...
func (fspg *FlexServicePoolGroup) NewFlexServicePoolGroup() {
	fspg.poolMap = make(map[string]map[string]*ServicePool)
...
// 添加或者更新一个ServicePool
func (fspg *FlexServicePoolGroup) UpdateServicePool(maxPoolSize int, groupId string, addr *ServiceManagerGroupAddr) {
	ipAndPort := merge_ip_and_port(addr.Ip, addr.Port)
	fspg.DeleteServicePool(groupId, addr)

	var servicePool *ServicePool = new(ServicePool)
	servicePool.NewServicePool(maxPoolSize, groupId, addr)
	if fspg.poolMap[groupId] == nil {
		fspg.poolMap[groupId] = make(map[string]*ServicePool)
	}

	fspg.poolMap[groupId][ipAndPort] = servicePool
...
func (fspg *FlexServicePoolGroup) DeleteServicePool(groupId string, addr *ServiceManagerGroupAddr) {
	...
	if fspg.poolMap[groupId] != nil {
		{ // 销毁原来的服务连接
			var oldServicePool *ServicePool = nil
...
// 根据groupId获取一个数据库连接
func (fspg *FlexServicePoolGroup) GetConnection(groupId string) *FlexRpcClient {
	if fspg.poolMap[groupId] == nil {
		return nil
	}
...
// 释放一个rpc连接
func (sspg *FlexServicePoolGroup) ReleaseConnection(fpc *FlexRpcClient) {
...
func (sp *ServicePool) NewServicePool(maxPoolSize int, groupId string, addr *ServiceManagerGroupAddr) {
	sp.Init()
	sp.MaxPoolSize = maxPoolSize
	sp.ObjectFactory = &RpcObjectFactory{groupId, addr}
	sp.TryTimes = SERVICE_MS_POOL_TRY_TIMES
	sp.GroupId = groupId
	sp.IpAndPort = merge_ip_and_port(addr.Ip, addr.Port)
	sp.Addr = addr
	sp.CreateObjects(maxPoolSize)
}

//从连接池中获取一个RPC连接
func (sp *ServicePool) GetConnection() *FlexRpcClient {
	obj := sp.Get()
	if obj != nil {
		return obj.(*FlexRpcClient)
	}
	return nil
}

//释放RPC连接到连接池
func (sp *ServicePool) ReleaseConnection(spc *FlexRpcClient) {
	sp.Release(spc)
}

type RpcObjectFactory struct {
	GroupId string
	Addr    *ServiceManagerGroupAddr
}

// 创建对象，例如：数据库连接
func (rpc *RpcObjectFactory) Create() interface{} {
	//log4go.InfoLog("  Create()   ........   GroupId=%s, Addr=%s   \n", rpc.GroupId, rpc.Addr.Json())
	log4go.InfoLog(message.INF_MSSV_09001, rpc.GroupId, rpc.Addr.Json())
...
// 验证对象是否有效（或运行正常）
func (rpc *RpcObjectFactory) Valid(obj interface{}) bool {
	if obj != nil {
		sc := obj.(*FlexRpcClient)
		return sc.Ping()
	}
	return false
}
...
func (rpc *RpcObjectFactory) ReleaseStart(obj interface{}) {

}

// AbstractPool的Release方法，释放对象后调用
func (rpc *RpcObjectFactory) ReleaseEnd(obj interface{}) {

}

// AbstractPool的Get方法，获取对象前调用
func (rpc *RpcObjectFactory) GetStart() {

}

// AbstractPool的Get方法，获取对象后调用
func (rpc *RpcObjectFactory) GetEnd(obj interface{}) {
