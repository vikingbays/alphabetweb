...
func serveFile(w http.ResponseWriter, r *http.Request, fs FileSystem, name string, redirect bool, rootAppName string) {
	...
	if d.IsDir() {
		...
		//dirList(w, f)     // 如果目录下有文件就展现文件列表，因为不安全，所以屏蔽掉
		//http.NotFound(w, r)
		Page404(w, r, rootAppName)
		return
	}
...
func FileServer(root FileSystem, rootAppname string) http.Handler {
	return &fileHandler{root, rootAppname}
}
...
func (f *fileHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	serveFile(w, r, f.root, path.Clean(upath), true, f.rootAppname)
}
